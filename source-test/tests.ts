...
interface TestData {
  strings: TestDataItem[];
  domains: TestDataItem[];
  ucs2: TestDataUcs2Item[];
  separators: TestDataItem[];
}
...
describe('punycode.ucs2.encode', function() {
  ...
  const codePoints: number[] = [0x61, 0x62, 0x63];
  const result: string = punycode.ucs2.encode(codePoints);
